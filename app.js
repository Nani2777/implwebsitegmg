...
app.post("/gmgfalconide", function(req, res) {
  ...
  var url =
    "http://evbk.gamooga.com/ev/?c=" + comp_id + "&v=" + visid + "&e=" + event;
  ...
  try {
    ...
    if (typeof data == "object") {
      if (data.jobname && data.jobname !== "null") {
        ...
        var Server =
          params.comp_id == "107a3b41-1aa3-45c6-a324-f0399a2aa2af"
            ? "js3in1.gamooga.com"
            : "evbk.gamooga.com";
        var url =
          "http://" +
      ...
      webhookData.forEach(function(each) {
        if (
          ...
          try {
            ...
            var event =
              "_email_" +
              (each["EVENT"] == "sent" ? "delivered" : each["EVENT"]);
...
app.get("/karvywebhook", function(req, res) {
  try {
    ...
    if (typeof data == "object") {
      ...
      try {
        var url =
          "http://evbk.gamooga.com/ev/?c=107a3b41-1aa3-45c6-a324-f0399a2aa2af&v=" +
          vid +
          "&e=" +
          event;
        Object.entries(camp_data).forEach(
...
app.post("/karvymailkootwebhook", function(req, res) {
  ...
  if (typeof webhookData == "object") {
    console.log(webhookData);
    if (
      (webhookData["event_type"] == "delivery_attempt" &&
        webhookData["status"] == "success") ||
      webhookData["event_type"] == "bounce_all"
    ) {
      try {
...
app.post("/stepwebhooksms", function(req, res) {
  try {
    ...
    if (typeof webhookData == "object") {
      ...
      var qury = req.query;
      var event =
        "_sms_" + (webhookData["Status"] == "sent" ? "delivered" : "failed");
